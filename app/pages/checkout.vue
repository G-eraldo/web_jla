  ...
  <section class="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-12">
    ...
    <form v-if="cart.items.length" class="mt-8 grid gap-8 lg:mt-10 lg:grid-cols-[1fr_.7fr] lg:gap-10" @submit.prevent="checkout">
      <div class="space-y-10">
        ...
        <fieldset><legend class="font-serif text-2xl sm:text-3xl">Livraison</legend><div class="mt-6 grid gap-3 sm:grid-cols-2"><label class="flex cursor-pointer gap-3 border p-4" :class="delivery.method === 'home' ? 'border-[#9b712d] bg-[#fdf7f2]' : 'border-[#e9ddd3]'"><input v-model="delivery.method" value="home" type="radio" name="delivery-method"><span><strong class="block font-medium">À domicile</strong><span class="mt-1 block text-sm text-[#776b64]">À l’adresse indiquée.</span></span></label><label class="flex cursor-pointer gap-3 border p-4" :class="delivery.method === 'pickup' ? 'border-[#9b712d] bg-[#fdf7f2]' : 'border-[#e9ddd3]'"><input v-model="delivery.method" value="pickup" type="radio" name="delivery-method"><span><strong class="block font-medium">Mondial Relay</strong><span class="mt-1 block text-sm text-[#776b64]">Choisissez un point relais disponible.</span></span></label></div><div v-if="delivery.method === 'pickup'" class="mt-4"><p v-if="!/^\d{5}$/.test(form.postalCode)" class="text-sm text-[#776b64]">Renseignez d’abord votre code postal pour voir les points relais disponibles.</p><p v-else-if="relayLoading" class="text-sm text-[#776b64]">Recherche des points relais…</p><p v-else-if="relayError" class="text-sm text-red-600">{{ relayError }}</p><div v-else class="grid gap-3"><button v-for="point in relayPoints" :key="point.id" type="button" class="border p-4 text-left" :class="selectedRelayId === point.id ? 'border-[#9b712d] bg-[#fdf7f2]' : 'border-[#e9ddd3]'" @click="selectRelay(point)"><span class="flex items-start gap-3"><input :checked="selectedRelayId === point.id" type="radio" name="relay-point" class="mt-1"><span><strong class="block">{{ point.name }}</strong><span class="mt-1 block text-sm text-[#776b64]">{{ point.address.join(', ') }}<template v-if="point.address.length">, </template>{{ point.postalCode }} {{ point.city }}<template v-if="point.distance"> · à {{ Math.round(point.distance / 1000 * 10) / 10 }} km</template></span></span></span></button></div></div></fieldset>
      </div>
      <aside class="h-fit bg-[#f5eee6] p-5 sm:p-8"><h2 class="font-serif text-2xl sm:text-3xl">Votre écrin</h2><div class="my-6 border-y py-5"><div v-for="item in cart.items" :key="item.id" class="mb-3 flex justify-between gap-3 text-sm"><span>{{ item.name }} × {{ item.quantity }}</span><span class="shrink-0">{{ money(item.price * item.quantity) }}</span></div></div><div class="flex justify-between font-serif text-xl sm:text-2xl"><span>Total</span><span>{{ money(cart.total) }}</span></div><p class="mt-3 text-sm text-[#776b64]">Paiement sécurisé par Mollie. La confirmation vous sera envoyée après validation du paiement.</p><label class="mt-5 flex gap-3 text-sm leading-5"><input v-model="acceptedTerms" required type="checkbox" class="mt-1"><span>J’accepte les conditions générales de vente et reconnais avoir lu la politique de confidentialité.</span></label><p v-if="errorMessage" class="mt-5 text-sm text-red-600">{{ errorMessage }}</p><button class="mt-6 w-full bg-[#302722] py-4 text-xs uppercase tracking-widest text-white disabled:cursor-not-allowed disabled:opacity-60" :disabled="loading">{{ loading ? 'Redirection…' : 'Payer avec Mollie' }}</button></aside>
    </form>
