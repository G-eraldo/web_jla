<script setup>
import { amountUntilFreeShipping, SHIPPING_PRICES, shippingAmountFor } from '~/lib/shipping'
    ...
    <form v-if="cart.items.length" class="mt-8 grid gap-8 lg:mt-10 lg:grid-cols-[1fr_.7fr] lg:gap-10"
      ...
      <div class="space-y-10">
        <div>
          ...
          <p class="mt-3 text-sm leading-6 text-[#776b64]">Livraison limitée à la France métropolitaine. Paiement par
            carte bancaire via Mollie ; d’autres moyens peuvent être proposés par Mollie selon votre banque.</p>
          <div class="mt-6 grid gap-4 sm:grid-cols-2">
            <label class="text-sm">Prénom<input v-model="form.firstName" required autocomplete="given-name"
                maxlength="100" class="mt-2 w-full border p-4"></label>
            <label class="text-sm">Nom<input v-model="form.lastName" required autocomplete="family-name" maxlength="100"
                class="mt-2 w-full border p-4"></label>
            <label class="text-sm sm:col-span-2">E-mail<input v-model="form.email" required type="email"
                autocomplete="email" maxlength="254" class="mt-2 w-full border p-4"></label>
            <label class="text-sm sm:col-span-2">Téléphone<input v-model="form.phone" required type="tel"
                autocomplete="tel" maxlength="30" class="mt-2 w-full border p-4"></label>
            <label class="text-sm sm:col-span-2">Adresse<input v-model="form.addressLine1" required
                autocomplete="address-line1" maxlength="200" class="mt-2 w-full border p-4"></label>
            <label class="text-sm sm:col-span-2">Complément d’adresse (facultatif)<input v-model="form.addressLine2"
                autocomplete="address-line2" maxlength="200" class="mt-2 w-full border p-4"></label>
            <label class="text-sm">Code postal<input v-model="form.postalCode" required autocomplete="postal-code"
                maxlength="5" inputmode="numeric" class="mt-2 w-full border p-4"></label>
            <label class="text-sm">Ville<input v-model="form.city" required autocomplete="address-level2"
                maxlength="100" class="mt-2 w-full border p-4"></label>
          </div>
        </div>
        <fieldset>
          ...
          <div class="mt-6 grid gap-3 sm:grid-cols-2"><label class="flex cursor-pointer gap-3 border p-4"
              :class="delivery.method === 'home' ? 'border-[#9b712d] bg-[#fdf7f2]' : 'border-[#e9ddd3]'"><input
                v-model="delivery.method" value="home" type="radio" name="delivery-method"><span><strong
                  class="block font-medium">À domicile · {{ hasFreeShipping ? 'Offerte' : money(SHIPPING_PRICES.home)
                  }}</strong><span class="mt-1 block text-sm text-[#776b64]">À l’adresse indiquée, en France
              ...
              :class="delivery.method === 'pickup' ? 'border-[#9b712d] bg-[#fdf7f2]' : 'border-[#e9ddd3]'"><input
                v-model="delivery.method" value="pickup" type="radio" name="delivery-method"><span><strong
                  class="block font-medium">Mondial Relay · {{ hasFreeShipping ? 'Offerte' :
                    money(SHIPPING_PRICES.pickup) }}</strong><span class="mt-1 block text-sm text-[#776b64]">Choisissez un
                  point relais disponible.</span></span></label>
          </div>
          ...
          <div v-if="delivery.method === 'pickup'" class="mt-4">
            ...
            <div v-else class="grid gap-3">
              <label v-for="point in relayPoints" :key="point.id"
                ...
                <input v-model="selectedRelayId" :value="point.id" type="radio" name="relay-point" class="mt-1"
                  @change="selectRelay(point)">
                <span><strong class="block">{{ point.name }}</strong><span class="mt-1 block text-sm text-[#776b64]">{{
                  point.address.join(', ') }}<template v-if="point.address.length">, </template>{{
                      point.postalCode }} {{ point.city }}<template v-if="point.distance"> · à {{
                      Math.round(point.distance / 1000 * 10) / 10 }} km</template></span></span>
              </label>
            </div>
          </div>
        </fieldset>
      ...
      <aside class="h-fit bg-[#f5eee6] p-5 sm:p-8">
        ...
        <div class="space-y-2 border-b pb-5 text-sm">
          ...
          <div v-if="cart.discountAmount" class="flex justify-between text-green-700"><span>Réduction · {{
            cart.promoCode
              }}</span><span>− {{ money(cart.discountAmount) }}</span></div>
          <div class="flex justify-between"><span>Livraison {{ delivery.method === 'pickup' ? 'Mondial Relay' : 'à domicile' }}</span><span>{{ hasFreeShipping ? 'Offerte' : money(shippingAmount) }}</span></div>
        </div>
        <div class="mt-5 flex justify-between font-serif text-xl sm:text-2xl"><span>Total</span><span>{{
          money(totalAmount)
            }}</span></div>
        <p class="mt-3 text-sm text-[#776b64]">Paiement par carte bancaire, via la page sécurisée Mollie. Après
          validation,
          l’e-mail de confirmation comprendra votre récapitulatif, votre facture et les CGV applicables.</p>
        <p class="mt-3 text-xs leading-5 text-[#776b64]">Livraison prévue sous 3 à 6 jours ouvrés à compter de la
          confirmation
          du paiement.</p>
        <label class="mt-5 flex gap-3 text-sm leading-5"><input v-model="acceptedTerms" required type="checkbox"
            class="mt-1"><span>J’accepte les <NuxtLink class="font-medium underline underline-offset-4"
              to="/conditions-generales-de-vente" target="_blank">conditions générales de vente</NuxtLink> et reconnais
            avoir
            lu la <NuxtLink class="font-medium underline underline-offset-4" to="/politique-confidentialite"
              target="_blank">
              politique de confidentialité</NuxtLink>.</span></label>
        <p class="mt-3 text-xs leading-5 text-[#776b64]">Les champs de ce formulaire sont nécessaires pour exécuter la
          commande, le paiement et la livraison. Maison JLA est responsable du traitement ; Mollie, Resend, Sendcloud et
          le
          transporteur reçoivent seulement les données utiles à leur mission. Vos droits s’exercent à
          contact@maisonjla.fr.
          Les détails, durées de conservation et transferts éventuels figurent dans la politique de confidentialité.</p>
