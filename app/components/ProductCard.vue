  ...
  <NuxtLink :to="`/produits/${product.slug}`"
    ...
    <div class="relative overflow-hidden bg-[#f0e8e0]">
      ...
      <span aria-hidden="true"
        class="absolute bottom-3 right-3 flex h-9 w-9 items-center justify-center rounded-full bg-[#fffaf5]/95 text-[#382b25] transition-transform group-hover:translate-x-0.5"><svg
          class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
          <path d="M5 12h14m-5-5 5 5-5 5" />
        </svg></span>
    </div>
    <div class="pt-3.5 sm:pt-4">
      <p class="text-[9px] uppercase tracking-[0.16em] text-[#887971]">{{ product.category }}</p>
      <h3
        class="mt-1.5 font-serif text-base leading-snug text-[#382b25] transition-colors group-hover:text-[#947756] sm:text-xl">
        {{ product.name }}</h3>
      <p class="mt-2 text-xs text-[#5f5047] sm:text-sm">{{ money(product.price) }}</p>
    </div>
  </NuxtLink>
</template>
