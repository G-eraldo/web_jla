...
export const useCartStore = defineStore('cart', {
  ...
  getters: {
    ...
    totalAfterDiscount() {
      return Math.max(0, Math.round((this.total - this.discountAmount) * 100) / 100)
    }
  },
  actions: {
    queuePromoRefresh() {
      ...
      this.promotion = null
      this.promoMessage = ''
      if (typeof window !== 'undefined') void this.applyPromoCode(this.promoCode, { silent: true })
    },
    addItem(product) {
    ...
    async applyPromoCode(input, { silent = false } = {}) {
      const code = String(input || '').trim().toUpperCase().slice(0, 40)
