...
export default defineEventHandler(async event => {
  ...
  if (Number(getHeader(event, 'content-length') || 0) > 20_000) throw createError({ statusCode: 413, message: 'La demande est trop volumineuse.' })
  ...
  if (!lines.length || requiredCustomerFields.some(field => !String(customer[field] || '').trim() || String(customer[field]).trim().length > 200)) throw createError({ statusCode: 400, message: 'Veuillez compléter vos informations de livraison.' })
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(customer.email).trim()) || !/^\d{5}$/.test(String(customer.postalCode).trim())) throw createError({ statusCode: 400, message: 'Vos coordonnées de livraison sont invalides.' })
  if (!['home', 'pickup'].includes(delivery.method) || !isValidPickupSelection(delivery)) throw createError({ statusCode: 400, message: 'Veuillez sélectionner un point relais Mondial Relay.' })
  if (body?.acceptedTerms !== true) throw createError({ statusCode: 400, message: 'Vous devez accepter les conditions générales de vente.' })
  ...
  if (delivery.method === 'pickup') {
    try {
      ...
      throw createError({ statusCode: error.statusCode || 502, message: error.message })
    }
  }
  ...
  try {
    ...
    throw createError({ statusCode: 503, message: 'Le catalogue est temporairement indisponible. Veuillez réessayer.' })
  }

  if (lines.some(line => line.quantity > 10)) throw createError({ statusCode: 400, message: 'La quantité maximale par bijou est de 10 exemplaires.' })
  ...
  try {
    ...
    throw createError({ statusCode: error?.statusCode || 503, message: error?.data?.error?.message || 'La réservation du stock est temporairement indisponible.' })
  }
  ...
  try {
    ...
    throw createError({ statusCode: 502, message: 'Le paiement est temporairement indisponible. Aucun montant n’a été débité.' })
  }
