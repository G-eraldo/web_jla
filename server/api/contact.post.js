...
export default defineEventHandler(async event => {
  enforceRateLimit(event, { name: 'contact', limit: 5, windowMs: 60 * 60 * 1000 })
  ...
  if (!config.resendApiKey || !config.resendFrom) {
    throw createError({
      statusCode: 503,
      statusMessage: 'Le formulaire est temporairement indisponible. Écrivez à maisonjla@outlook.fr.'
    })
  }
  ...
  try {
    ...
    throw createError({
      statusCode: 502,
      statusMessage: 'Le message n’a pas pu être envoyé. Vous pouvez écrire à maisonjla@outlook.fr.'
    })
  }
