...
export default defineEventHandler(async event => {
  ...
  if (!referencePattern.test(reference)) throw createError({ statusCode: 400, message: 'Référence de commande invalide.' })
  ...
  if (orderDocumentId(payment.metadata) !== order.documentId) throw createError({ statusCode: 409, message: 'Le paiement ne correspond pas à cette commande.' })
